...
assign initial_pop = (frmvalrdy & frm_sof_d[1]) | (frmvalrdy & frm_eol & frm_first_line);
...
always@(posedge clk or negedge rst_n)
  if(~rst_n                              ) pix_in_line <= 'd0                              ; else
  if(frm_first_line & frm_eol & frmvalrdy) pix_in_line <= fifo_usedwords + fifo_push + 2'd2;      // Number of pixels in a line, compensate the initial pop
  
always@(posedge clk or negedge rst_n)
  if(~rst_n                                          ) window_cnt <= 9'd0                             ; else
  if(frm_first_line & frm_eol & frmvalrdy            ) window_cnt <= fifo_usedwords + fifo_push + 2'd2; else // Load on first eol
  if(((~frm_first_line) & (~|window_cnt) & winvalrdy)) window_cnt <= pix_in_line                      ; else // Load when not frm first line and the counter is 0
  if(winvalrdy                                       ) window_cnt <= window_cnt - 1'b1                ;      // Decrement at each valid output
...
always@(posedge clk or negedge rst_n)
  ...
  if(last_line | frm_sof) valrdy_cnt <= 2'd0                              ; else
  if(~frm_first_line    ) valrdy_cnt <= valrdy_cnt + frmvalrdy - winvalrdy;      // Count the number of new elements in the 3 input registers, increment when pipe is enabled decrement when data recieved
...
always@(posedge clk or negedge rst_n)
  ...
  if(frm_first_line                                            ) fifo_pushdata <= {cfg_bkg   , line0_left}; else
...
always@(posedge clk or negedge rst_n)
  ...
  if(fifo_empty) fifo_pop <= 1'b0                                                                ; else // Reset when fifo is empty or is at the last element
                 fifo_pop <= ((~frm_first_line) & frmvalrdy) | initial_pop  | (last_line & win_rdy);      //Pop at first eol and sol with no sof to prepare the data
                                                                                                        // Always active at last line and pop at input valrdy while not in first line   
  						
always@(posedge clk or negedge rst_n)
  if(~rst_n  ) fifo_push <= 1'b0     ; else
  if(~frm_sof) fifo_push <= frmvalrdy | (last_line & winvalrdy & (window_cnt == 2'd2) & (~last_push));
			   
always@(posedge clk or negedge rst_n)
  if(~rst_n             ) fifo_clr <= 1'b0; else
  if(fifo_clr           ) fifo_clr <= 1'b0; else
  if(win_eof & winvalrdy) fifo_clr <= 1'b1; 
//----------------------------- Output frame interface control signal logic ---------------------------------  
//Valid signal
always@(posedge clk or negedge rst_n)
  if(~rst_n                                          ) win_val <= 1'b0                                      ; else
  if(winvalrdy & win_eof & last_line | frm_first_line) win_val <= 1'b0                                      ; else
  if(last_line                                       ) win_val <= 1'b1                                      ; else // Last line alway valid, no output dependence
...
always@(posedge clk or negedge rst_n)
  ...
  if(last_line                      ) frm_rdy <= 1'b0; else
                                      frm_rdy <= ((frm_first_line & ~fifo_full) |            //At first line ready while there is room in the fifo
								                  (valrdy_cnt + frmvalrdy - winvalrdy < 2)) //Or the input registers are not populated
								        		& (~last_line) & in_frm;                   // And not in last ready when generating last line and in current frame flag is active
...
always@(posedge clk or negedge rst_n)
  ...
  if(winvalrdy & win_eol & (~fifo_empty)) win_sol <= 1'b1;      // Set at window counter reset
...
always@(posedge clk or negedge rst_n)
  ...
  if(last_line & fifo_empty & set_eol) win_eof <= 1'b1;      // Set at last line when fifo is empty 
...
always@(posedge clk or negedge rst_n)
  ...
  if(pipe_en & (~frm_first_line)) begin
